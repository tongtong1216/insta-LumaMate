...
随后真机可访问 `http://127.0.0.1:8000/health`。当前 App 已具备
`INTERNET` 权限、本地明文 HTTP 配置，以及
`DBackendSceneAnalysisClient` 适配器；业务层仍需要在后台线程中传入真实
`AnalyzeSceneRequest`，并把响应交给 `V1SceneSemanticDataSource` 和
`PolicyCoordinator`。模拟器访问后端成功不等于相机 SDK 已接入。
